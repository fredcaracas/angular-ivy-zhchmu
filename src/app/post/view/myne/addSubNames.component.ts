...
@Component({
  selector: 'app-addSubNetwork',
  templateUrl: './addSubNames.component.html',
  styleUrls: ['./addSubNames.component.css']
})
export class AddSubNetworkNameComponent implements OnInit {
  ...
  constructor(
    ...
    public thisDialogRef: MatDialogRef<AddSubNetworkNameComponent>
  ) {
  ...
  saveSubName() {
    ...
    arry.subscribe(response => {
      ...
      if (duplicateElements.length > 0) {
        ...
        let errormessage = "Sub Network already exists."
        this.alertpopup(errormessage);

      } else {

        let resp = this.service.addSubNetwokName(this.model.name);
        ...
        resp.subscribe(res => {
          ...
          const dialogRef = this.dialog.open(RecordSaveComponent, {
            ...
            width: '600px',
            disableClose: true,
            data: "✔️ New Sub Network Name has been saved."
          });
